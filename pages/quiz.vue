<template>
  <div class="min-h-screen bg-gradient-to-br from-blue-900 via-blue-950 to-black">
    <!-- Navbar -->
    ...
    <main class="relative flex items-center justify-center p-4 pt-20" style="min-height: calc(100vh - 64px)">
      ...
      <div v-else>
        <div class="max-w-2xl w-full bg-black/50 backdrop-blur-sm border border-white/10 rounded-xl p-8 shadow-xl">
          <div class="space-y-8">
            <div class="space-y-4">
            ...
            <div class="space-y-3">
              <button
                v-for="(option, index) in questions[currentQuestion]?.propositions || []"
                :key="index"
                @click="toggleAnswer(index)"
                class="w-full text-left px-6 py-4 rounded-lg transition-all"
                :class="{
                  'bg-blue-600 text-white': selectedAnswers.includes(index),
                  'bg-blue-800 text-white hover:bg-gray-700': !selectedAnswers.includes(index)
                }"
              >
                {{ option }}
              </button>
...
const QuestionnaryPost = async (selectedAnswers, indexQuestion) => {
  ...
  try {
      ...
      const response = await fetch(BASE_API_URL + "/datas/serie/last", {        //  <<<< backend api here <<<
        method: "POST",
        headers: {
...
const submitAnswer = () => {
  QuestionnaryPost(selectedAnswers.value, currentQuestion.value).then((res) => console.log("res: ", res))
